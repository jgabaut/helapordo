...
void init_floor_rooms(Floor* floor) {
    ...
    for (int x = 0; x < FLOOR_MAX_COLS; x++) {
        for (int y = 0; y < FLOOR_MAX_ROWS; y++) {
            floor->roomclass_layout[x][y] = floor->floor_layout[x][y] == 1 ? BASIC : -1;
        }
    }
}
...
void load_floor_explored(Floor* floor) {
    ...
    for (int x = 0; x < FLOOR_MAX_COLS; x++) {
        for (int y = 0; y < FLOOR_MAX_ROWS; y++) {
            floor->explored_matrix[x][y] = floor->floor_layout[x][y] == 1 ? 0 : -1;
        }
    }
}
...
void display_roomclass_layout(Floor* floor, WINDOW* win) {
    if (win == NULL ) {
	log_tag("debug_log.txt","[ERROR]","display_roomclass_layout:  win was NULL");
	exit(EXIT_FAILURE);
    }
    for (int y = 0; y < FLOOR_MAX_ROWS; y++) {
        for (int x = 0; x < FLOOR_MAX_COLS; x++) {
...
void display_floor_layout(Floor* floor, WINDOW* win) {
    if (win == NULL) {
	    log_tag("debug_log.txt","[ERROR]","display_floor_layout():  win was NULL.");
	    exit(EXIT_FAILURE);
    }
    int isFull = -1;
    int isColored = -1;
...
void display_explored_layout(Floor* floor, WINDOW* win) {
    if (win == NULL) {
	    log_tag("debug_log.txt","[ERROR]","display_explored_layout():  win was NULL.");
	    exit(EXIT_FAILURE);
    }
    int isWalkable = -1;
    int isExplored = -1;
...
void floor_random_walk(Floor* floor, int x, int y, int steps, int do_layout_clean) {
    ...
    do {
		...
		if (new_x > 0 && new_x < (FLOOR_MAX_COLS-1) && new_y > 0 && new_y < (FLOOR_MAX_ROWS-1)) {
...
void floor_set_room_types(Floor* floor) {
    ...
    */
    ...
    do {
      if (shop_attemps == 0) {
	      log_tag("debug_log.txt","[ERROR]","floor_set_room_types():  failed generating a distant SHOP room in 100 attempts. Just give me a valid one.");
	      ...
	      do {
	        ...
	        shop_x = rand() % max_x+1;
	        shop_y = rand() % max_y+1;
	      } while (floor->floor_layout[shop_x][shop_y] != 1&& (shop_x != center_x || shop_y != center_y));
      } else {
        // Select a valid random room as the SHOP
        shop_x = rand() % max_x+1;
        shop_y = rand() % max_y+1;
    ...
    } while (floor->floor_layout[shop_x][shop_y] != 1 || calc_distance(shop_x, shop_y, center_x, center_y) < 2 );
    ...
    do {
      if (boss_attemps == 0) {
	      log_tag("debug_log.txt","[ERROR]","floor_set_room_types():  failed generating a distant BOSS room in 100 attempts. Just give me a valid one.");
	      //TODO: enable logging of insidious floor setups.
	      //debug_print_floor_layout(floor, logfile);
	      //fprintf(logfile,"Area is [ %i ]", floor->area);
	      do {
	        ...
	        boss_x = rand() % max_x+1;
	        boss_y = rand() % max_y+1;
	      } while (floor->floor_layout[boss_x][boss_y] != 1 && (boss_x != center_x || boss_y != center_y) && (boss_x != shop_x || boss_y != shop_y) );
      } else {
        // Select a valid random room as the SHOP
        boss_x = rand() % max_x+1;
        boss_y = rand() % max_y+1;
    ...
    } while (floor->floor_layout[boss_x][boss_y] != 1 || (calc_distance(boss_x, boss_y, center_x, center_y) < 4 ) || (calc_distance(boss_x, boss_y, shop_x, shop_y) < 4));
    ...
    if (treasure_roll >= 25) {
	    ...
	    do {
	      if (treasure_attemps == 0) {
		      log_tag("debug_log.txt","[ERROR]","floor_set_room_types():  failed generating a distant TREASURE room in 100 attempts. Just give me a valid one.");
		      //TODO: enable logging of insidious floor setups.
	      	      //debug_print_floor_layout(floor, logfile);
	      	      //fprintf(logfile,"Area is [ %i ]", floor->area);
		      do {
			...
			treasure_y = rand() % max_y+1;
		      } while (floor->floor_layout[treasure_x][treasure_y] != 1 && (treasure_x != center_x || treasure_y != center_y) && (treasure_x != shop_x || treasure_y != shop_y) && (treasure_x != boss_x || treasure_y != boss_y) );
		      break;
	      } else {
	      	// Select a valid random room as the TREASURE
	      	treasure_x = rand() % max_x+1;
	    ...
	    } while (floor->floor_layout[treasure_x][treasure_y] != 1 || (calc_distance(treasure_x, treasure_y, center_x, center_y) < 4 ) || (calc_distance(treasure_x, treasure_y, shop_x, shop_y) < 4) || (calc_distance(treasure_x, treasure_y, boss_x, boss_y) < 4) );
	...
	log_tag("debug_log.txt","[FLOOR]","Tried and failed treasure generation.");
	exit(EXIT_FAILURE);
    }
	...
	do {
	  enemy_x = rand() % max_x;
	  enemy_y = rand() % max_y;
	} while (floor->floor_layout[enemy_x][enemy_y] != 1 && floor->roomclass_layout[enemy_x][enemy_y] != BASIC);
...
void draw_cell(Floor* floor, int cell_x, int cell_y, WINDOW* win, int drawcorner_x, int drawcorner_y, int x_size, int y_size, int recurse) {
	...
	if (floor->floor_layout[cell_x][cell_y] == 0) {
		...
		if (recurse > 0 && cell_x < FLOOR_MAX_COLS-1) draw_cell(floor,cell_x+1,cell_y,win,drawcorner_x+3,drawcorner_y,x_size,y_size, recurse-1);
			...
			if (floor->roomclass_layout[target_x][target_y] != WALL && floor->floor_layout[target_x][target_y] > 0) {
			  ...
			  *current_y = target_y;
			  draw_floor_view(floor, *current_x, *current_y, win);
				...
				picked = 0;
				log_tag("debug_log.txt","[DEBUG]","Bonked in a wall in move_update().");
				continue;
			}
